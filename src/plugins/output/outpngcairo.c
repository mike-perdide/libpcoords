...
static void draw_text(double x, double y, char *color, double size, char *text)
{
	cairo_set_source_rgb(cr,
			     pcoords_color_extract_r(color),
			     pcoords_color_extract_g(color),
			     pcoords_color_extract_b(color));
...
void draw_line(PicvizImage *image, PcvID axis_id, PicvizLine *line, PicvizAxisPlot *axisplot1, PicvizAxisPlot *axisplot2, PcvWidth x1, PcvHeight y1, PcvWidth x2, PcvHeight y2)
{
	...
	switch(engine.heatline_algo) {
		case 0: /* Default */
			color = pcoords_properties_get(axisplot1->props, "color");
			break;
		case 1: /* Virus */
	...
	if (!color) color = pcoords_properties_get(line->props, "color");
	...
	line_width = atof(pcoords_properties_get(line->props, "penwidth")) * 0.4;
	if (line_width) {
		cairo_set_line_width (cr, atof(pcoords_properties_get(line->props, "penwidth")));
	}
...
void output(PicvizImage *image, char *arg)
{
	...
	cairo_set_source_rgba (cr,
			       pcoords_color_extract_r(image->bgcolor),
			       pcoords_color_extract_g(image->bgcolor),
			       pcoords_color_extract_b(image->bgcolor),
			       image->bgalpha);
	cairo_fill (cr);
	...
	while (image->axesorder[counter]) {
		...
		cairo_line_to(cr, (double) pcoords_axis_position_get(counter), 
			      (double)image->height);
		counter++;
	}
	cairo_stroke(cr);
	...
	if (image->header_height) {
		...
		while (image->axesorder[counter]) {
			...
			char *label = pcoords_properties_get(axis->props, "label");
			cairo_text_extents(cr, label, &extents);
			draw_text(pcoords_axis_position_get(counter) - (extents.width/2), 
